...
bot.on(message("sticker"), (ctx) =>
  ctx.reply("смотреть аниме", "вы ввели не число, введите число."),
);
...
bot.on(message("text"), async (ctx) => {
  ...
  if (ctx.session.wait === "support") {
    ...
    await ctx.telegram.sendMessage(
      process.env.CHAT_ID,
      `Сообщение от ${ctx.chat?.first_name ?? ""} ${
        ctx.chat?.last_name ?? ""
      }, @${ctx.chat?.username}`,
    );
  ...
  if (ctx.message.text === "🎫 участвовать в Q ЛОТТО") {
    ...
    await ctx.reply(
      "розыгрыш пройдёт 31 декабря в 18:00 по мск на канале @slovo_jokera",
    );
  ...
  if (ctx.session.wait === "number") {
    ...
    if (checkCorrectNumber(Number(ctx.message.text))) {
      ...
      const stream = await addTextToImage(
        ctx.message.text,
      ...
      return ctx.reply(
        "отправьте ваш билет в лс Джокеру @JoJoker_law для подтверждения",
      );
    } else {
